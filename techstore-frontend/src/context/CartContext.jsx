...
function loadCart(userId) {
  ...
  try {
    ...
    return {
      ...
      promoMeta: promoMeta?.valid ? promoMeta : null,
    };
  } catch {
    return EMPTY_STATE;
  }
...
function cartReducer(state, action) {
  switch (action.type) {
    ...
    case CART_ACTIONS.UPDATE_QTY:
      return {
        ...state,
        items: updateItemQty(state.items, action.id, action.qty),
      };
    ...
    case CART_ACTIONS.APPLY_PROMO: {
      ...
      return {
        ...state,
        promoCode: action.code.trim().toUpperCase(),
        promoMeta: result,
      };
    }
...
export function CartProvider({ children }) {
  ...
  const addToCart = useCallback(
    (product) => dispatch({ type: CART_ACTIONS.ADD, product }),
    [],
  );
  const removeFromCart = useCallback(
    (id) => dispatch({ type: CART_ACTIONS.REMOVE, id }),
    [],
  );
  const updateQty = useCallback(
    (id, qty) => dispatch({ type: CART_ACTIONS.UPDATE_QTY, id, qty }),
    [],
  );
  ...
  const clearPromo = useCallback(
    () => dispatch({ type: CART_ACTIONS.CLEAR_PROMO }),
    [],
  );
