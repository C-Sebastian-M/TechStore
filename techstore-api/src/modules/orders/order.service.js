...
import { getDiscount, incrementUsage }   from './promo.service.js'

// D: constantes vienen de entorno
...
export async function createOrder(
  userId,
  { items, shipping, paymentMethod, promoCode },
) {
  return prisma.$transaction(async (tx) => {
    ...
    // Lee el descuento desde BD (PromoCode) — ya no hardcodeado
    const discountRate       = await getDiscount(promoCode)
    ...
    // Contabilizar el uso del código promocional
    await incrementUsage(promoCode)
    ...
    prisma.order.findMany({
      ...
      include: {
        items: {
          include: {
            product: {
              select: { name: true, brand: true, image: true, price: true },
            },
          },
        },
      },
