...
export async function getDashboardStats() {
  ...
  const [
    ...
    prisma.product.findMany({ where: { stock: { lte: 5 }, isActive: true }, select: { id: true, name: true, brand: true, stock: true, image: true }, orderBy: { stock: 'asc' }, take: 8 }),
    prisma.order.findMany({ take: 8, orderBy: { createdAt: 'desc' }, include: { user: { select: { name: true, email: true } }, items: { select: { qty: true } } } }),
  ...
  return {
    ...
    orders:       { total: totalOrders, month: monthOrders, byStatus: ordersByStatus.reduce((acc, r) => { acc[r.status] = r._count.id; return acc }, {}) },
...
export async function listUsers({ page = 1, limit = 20, search, role } = {}) {
  ...
  const [users, total] = await Promise.all([
    prisma.user.findMany({
      ...
      select:  { id: true, email: true, name: true, phone: true, role: true, createdAt: true, _count: { select: { orders: true, favorites: true } } },
    }),
...
export async function changeUserRole(adminId, targetId, role) {
  ...
  return prisma.user.update({ where: { id: targetId }, data: { role }, select: { id: true, name: true, email: true, role: true } })
}
...
export async function listAllOrders({ page = 1, limit = 20, status, search } = {}) {
  const where = {
    ...(status && { status }),
    ...(search && { OR: [{ orderNumber: { contains: search, mode: 'insensitive' } }, { user: { name: { contains: search, mode: 'insensitive' } } }, { user: { email: { contains: search, mode: 'insensitive' } } }] }),
  }
  const [orders, total] = await Promise.all([
    prisma.order.findMany({
      where, skip: (page - 1) * limit, take: limit,
      orderBy: { createdAt: 'desc' },
      include: { user: { select: { name: true, email: true } }, items: { include: { product: { select: { name: true, image: true } } } } },
    }),
...
export async function getOrderDetail(id) {
  const order = await prisma.order.findUnique({
    where:   { id },
    include: { user: { select: { name: true, email: true, phone: true } }, items: { include: { product: { select: { name: true, brand: true, image: true, price: true } } } } },
  })
  if (!order) {
